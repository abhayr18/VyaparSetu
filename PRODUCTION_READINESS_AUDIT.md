...
- **P0.6 (money as floating point) — resolved in Phase 1 (2026-08-26), see below.** Phase 0's paise-rounding at each step contained the drift; integer-paise storage now removes it at the source, so reconciliation compares balances to their ledger with `=` rather than a half-paisa tolerance.
...
Test suite: **137 passing, 9 files** (`cd backend && npm test`), up one from Phase 0 — `schema.test.js` gained two storage-class assertions and `reconciliation.test.js` dropped its now-impossible "sub-paise float noise" case.

| Gap | Status | What changed |
|---|---|---|
| **P0.6** money stored as floating point | Fixed | Money is whole integer paise on disk, rupees everywhere in JS, with one conversion seam in `backend/utils/money.js` — `toPaise`/`toRupees`, a per-table `MONEY_FIELDS` map, and `rowToRupees`/`rowToPaise` row helpers so each model read/write is a one-liner. Every money column is `INTEGER` in `init.js`; migration 8 rebuilds the six money-bearing tables with `CAST(ROUND(col * 100) AS INTEGER)`, and returns early when the columns are already `INTEGER` so a fresh install does no table churn. `commission_rate` (a percentage), `discount_value` (dual-unit) and `weight`/`quantity` (kilograms) deliberately stay `REAL` — ×100 would corrupt them. Because every write rounds to paise, no un-rounded float can reach a money column, and the delete-refund path (`credit_balance − SUM(amount)`) stays integer-to-integer. Reconciliation's tolerance drops to `0`: a balance and its ledger are both exact integers. **Storage-only** — the HTTP API, every screen, and the pure calculation utilities (`utils/calculation.js`, `utils/billingCalc.js`) are unchanged. |
